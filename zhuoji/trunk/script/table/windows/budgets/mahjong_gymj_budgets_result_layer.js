    ...
    GLOBAL_OBJ.table.windows.gymj.result.Budgets.Layer = GLOBAL_OBJ.bkernel.base.BaseWindowController.extend({
		...
		updateXZ:function(){
			...
			that['task_Node'].scheduleOnce(function () {
				if (MODEL_TASK_LOOPACVIVETASK.getLoopAcviveRoundCount() > 0) {  	// 赢几局
					GLOBAL_OBJ.LOGD(this._TAG," 任务 1111");
				...
				} else if (MODEL_TASK_LOOPWINTIMESTASK.getIsHaveLoopWinTimes()) {
					GLOBAL_OBJ.LOGD(this._TAG," 任务 2222");
					var _data = MODEL_TASK_LOOPWINTIMESTASK.loopAcviveTransition();
					var taskCell = new GLOBAL_OBJ.table.windows.gymj.result.Budgets.TaskCell();
					that['task_Node'].addChild(taskCell.getRootNode());
					if (_data.succeed) {
						taskCell.newTaskCell(_data);
				...
				} else if (MODEL_TASK_WINSTREAK.getIsHaveWinStreak() && MODEL_TASK_WINSTREAK.getIsHaveAward()) {
					GLOBAL_OBJ.LOGD(this._TAG," 任务 3333");
					if (MODEL_TASK_WINSTREAK.getWinStreaks()) {
						var taskCell = new GLOBAL_OBJ.table.windows.gymj.result.Budgets.TaskStreakCell();
						that['task_Node'].addChild(taskCell.getRootNode());
						taskCell.updateTaskCell();
					}
				}
			},1);
		},

...
(function(){
	...
	GLOBAL_OBJ.table.scenes.Table.Panels.Player = GLOBAL_OBJ.bkernel.base.BaseController.extend({
		...
		doSit:function(_uid, _callFunc){
			...
			var tableType = MODEL_TABLEINFO.getTableType();
			if (tableType == GLOBAL_OBJ.TableType.Match && that.seatId == GLOBAL_T.MYSEAT) {
				//比赛场头像下方会有实时名次显示，所以要把自己的头像向上移32像素
				this.portraitNode.y += 32;
			}
		...
		doinviteBtnVisible:function (isSeen) {
			if (this.inviteNode) {this.inviteNode.setVisible(isSeen);}
		},
		...
		/*
		万筒条按照每一门牌的数量从左向右排序:牌数量相同,按照 万 筒 条
		牌大小:1—9
		 */

		doSortMahjs:function(){
			var wan = 0;
			var tong = 0;
			var tiao = 0;
			for(var j = 0 ; j < this.mahjs.length; ++j){
				var tile = this.mahjs[i].getTile();
				// if (tile <= )
				if (tile >= 1 && tile <= 9) {
					wan = wan + 1;
			...
			for(var j = 0 ; j < this.mahjs.length; ++j){
				...
				// if (tile <= )
				if (tile >= 1 && tile <= 9) {
					this.mahjs[j].haveNum = wan;
				}else if(tile >= 11 && tile <= 19) {
					this.mahjs[j].haveNum = tong;
				}else if(tile >= 21 && tile <= 29) {
					this.mahjs[j].haveNum = tiao;
				}
			}
			...
			this.mahjs.sort(function(a, b) {
				if (a.isDingQue && !b.isDingQue) {
					...
					return true;
				}else if (Math.ceil(a.getTile() / 10) !== Math.ceil(b.getTile() / 10)) {
					return b.haveNum - a.haveNum
				} else {
					return b.getTile() - a.getTile();
				}
			});
		...
		doBudgetLiang: function(_tiles, _gameOver, callBackfun) {
			...
			if (this.seatId == GLOBAL_T.MYSEAT) {
				...
				if(!this.getHuState()){
					...
					GLOBAL_OBJ.LOGD(this._TAG, "doBudgetLiang 牌落下 ");
					for (var i = 0; i < this.mahjs.length; ++i) {
		...
		doBudgetDownMahj: function( _fake, _isFirst, _playmode, _winNum ) {
			...
			if (_isFirst && !_fake)
			{
				AUDIO.audio(GLOBAL_OBJ.RES.ZHUOJI_EFFECT_EFFECT_TUIDAO_MP3);
			}
			var mjss;

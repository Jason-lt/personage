...
(function(){
	...
	GLOBAL_OBJ.table.scenes.Table.Scene = GLOBAL_OBJ.bkernel.base.BaseSceneController.extend({
		...
		onLoad: function() {
            ...
            this['table_bg'].y = size.height / 2; //居中对齐
            ...
            */
            ...
            if (MODEL_TABLEINFO.getTableType() == GLOBAL_OBJ.TableType.Match){
                ...
                var curDes = GLOBAL_OBJ.businesses.scenes.Match.Model.getCurDes();

                if (curDes) {
                    if (curDes.type != GLOBAL_OBJ.MatchType.stage_match){
                        // var curRank = GLOBAL_OBJ.businesses.scenes.Match.Model.getCurRank();
                        var pars;
                        // if (curRank){
                        //     pars = curRank.rankName.split('/');
                        // }
                        // else{
                        var curWaitInfo = GLOBAL_OBJ.businesses.scenes.Match.Model.getCurWaitInfo();
                        if (curWaitInfo && curWaitInfo.rankName) {
                            pars = curWaitInfo.rankName.split('/');
                        }

                        // }

                        if (pars){
                            this.showMatchRank(pars[0], pars[1]);
                        }

                        GLOBAL_OBJ.bkernel.utils.Notification.listen(GLOBAL_OBJ.businesses.Events.UPDATE_MAHJ_MATCH_RANK, this.onMatchInfoUpdate, this);
                    }
                    else{
                        GLOBAL_OBJ.businesses.network.C2S.requestMatchSigns();
                        ty.Timer.cancelTimer(this, this.getUpdateInfo);
                        GLOBAL_OBJ.bkernel.utils.Notification.listen(GLOBAL_OBJ.businesses.Events.UPDATE_TIME_MATCH_INFO, this.onTimeMatchInfoUpdate, this);
                        ty.Timer.setTimer(this, this.getUpdateInfo, 20, cc.REPEAT_FOREVER, 0);
                        this.getUpdateInfo();
                    }
                }
            }
            GLOBAL_OBJ.businesses.utils.Timer.stop(this._TAG, "onLoad");

		},

        getUpdateInfo:function () {
            var curDes = GLOBAL_OBJ.businesses.scenes.Match.Model.getCurDes();
            GLOBAL_OBJ.businesses.network.C2S.requestMatchUpdate(curDes.roomId);
        },

        onMatchInfoUpdate:function (waitInfo) {
            var pars = waitInfo.rank.split('/');
            this.showMatchRank(pars[0], pars[1]);
        },

        onTimeMatchInfoUpdate:function (data) {
            if (!data.waitInfo) return;
            var pars = data.waitInfo.rank.split('/');
            this.showMatchRank(pars[0], pars[1]);
        },


        showMatchRank:function (curRank, allCount) {
            this['nodeRealRank'].setVisible(true);

            this['txtCurRank'].setString(curRank);
            this['txtAllCount'].setString('/' + allCount);

            this['txtAllCount'].x = this['txtCurRank'].width;
            this['nodeRealRankCtn'].width = this['txtAllCount'].x + this['txtAllCount'].width;
        },
        ...
        */
        ...
        sameIpTips:function () {
            ...
            GLOBAL_OBJ.bkernel.utils.Notification.listen(GLOBAL_OBJ.table.Events.UPDATE_TABLE_SIT, function(){
                if (MODEL_TABLEINFO.getShowSameIP() == 0) { // 添加配置,是否显示同一IP下提示;
                    GLOBAL_OBJ.LOGD(this._TAG, "sameIpTips ShowSameIP");
                    return;
                }
                GLOBAL_OBJ.LOGD(this._TAG, "sameIpTips UPDATE_TABLE_SIT;");
                var counts      = MODEL_TABLEINFO.getSeatCount();
                var samePlayers = {};
                for(var i = 0 ; i < counts ; ++i){
        ...
        onTaskBtn:function () {
            ...
            AUDIO.audio(GLOBAL_OBJ.RES.UI_CLICKBUTTON_MP3);
            this.taskPanel = GLOBAL_OBJ.bkernel.windows.Factory.produce(GLOBAL_OBJ.table.windows.consts.C_TABLE_TASK,
                {}, this.windowNode);
            GLOBAL_OBJ.LOGD(this._TAG, "onTaskBtn end;");
        },

        hideTaskPanel:function () {
            if (this.taskPanel && this.taskPanel.getRootNode()) {
                this.taskPanel.windowClose();
            }
        },

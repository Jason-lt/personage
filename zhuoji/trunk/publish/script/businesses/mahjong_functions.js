...
(function(){
	...
	GLOBAL_OBJ.businesses.functions = {
    	...
    	*/
		...
		getPreResCfg:function () {
			if (!this.preResCfg){
				...
				this.preResCfg = [
					...
					searchPach + "/img/mahj_match_details.png",
					searchPach + "/img/mahj_match_result.png",
					searchPach + "/img/mahj_match_wait.png",
				];
			}
		...
		isCanShareWeiXin:function () {
			var sysConfig = ty.SystemInfo.getConfig();
			if(sysConfig){
				GLOBAL_OBJ.LOGD("isCanShareWeiXin", "sysConfig.shareConfig:" + JSON.stringify(sysConfig.shareConfig));
			}
			return (sysConfig && typeof(sysConfig.shareConfig) == "undefined" || sysConfig.shareConfig.weixinShare);
		},
        ...
        getMahjPlugInVersion:function () {
            ...
            if(that.version){
				...
				GLOBAL_OBJ.LOGD("... plugInVersion 0: ", plugInVersion);
            }else {
				...
				GLOBAL_OBJ.LOGD("... plugInVersion 1: ", plugInVersion);
            }
		...
		// 异步下载，若存在本地缓存则直接从本地加载 (By Luwei)
		// @param: url - 要下载的资源文件的网址
		// @param: savepath - 下载后存储在哪里, 一般为：games/ddz/img/nopack/?/
		// @param: params - 传入的参数，此参数会传入到回调中
		// @param: callback - 下载完成后的回调
		// @param: lifeNode - 生命周期Node，异步下载回调的监听生命周期依附于此Node，若此Node被删除则取消监听(即不会回调)
		// 注意参数：回调函数callback有三个参数
		// callback(success, params, path)
		//  - success: 是否成功
		//  - params: 从asyncDownload()传入的参数
		//  - path: 资源下载后的路径，若失败则为空
		__asyncdownload_count__ : 0,

		asyncDownload : function(url, savepath, params, callback, lifeNode) {
			var dlkey = "mahj.asyncdownload" + this.__asyncdownload_count__++; // key must be different
			if( typeof(url) == 'undefined' || url.indexOf('http') < 0 ){
				GLOBAL_OBJ.LOGW('GLOBAL_OBJ.businesses.functions.asyncDownload', 'Invalid url to download, return!!!!!! url:' + url);
				return callback(false, params, null);
			}
			var controller = {
				onDownloaded: function(response, params) {
					ty.AsyncImgDownload.removeAsyncImgDownloadObserver(dlkey, controller, controller.onDownloaded);
					if (response.statusCode == 200 && response.openErr != 1 && response.writeErr != 1) { // success
						if (typeof(callback) == 'function') { callback(true, params, response["path"]); } return;
					}
					return callback(false, params, null); // download failed
				}
			};

			this.listenNodeCleanup(lifeNode, function() {
				ty.AsyncImgDownload.removeAsyncImgDownloadObserver(dlkey, controller, controller.onDownloaded);
				controller.onDownloaded = null;
			});

			ty.AsyncImgDownload.addAsyncImgDownloadObserver(dlkey, controller, controller.onDownloaded);
			ty.AsyncImgDownload.downloadImgAsync(dlkey, url, savepath, params);
		},

		listenNodeCleanup : function(node, callback) {
			if (node && callback) {
				var LifeProxyNode = cc.Node.extend({
					ctor: function() { this._super(); },
					onCleanup: function() { callback(); }
				});
				node.addChild(new LifeProxyNode);
				return true;
			}
			return false;
		},
		leftpad : function(str, width, char) {
			...
			var s = (char||' ').toString();
			for (var i = 1; i < width - str.length; i++) { s += char; }
			return s+str;
		},

...
(function(){
	...
	GLOBAL_OBJ.table.windows.Menu.Window = GLOBAL_OBJ.bkernel.base.BaseWindowController.extend({
		...
		onClickQuit:function(){
			...
			// GLOBAL_OBJ.LOGD("table_menu_onClickQuit :", reason);
			...
			if (this.isCreate){
				...
				if(this.menuState == "waitting"){//等待状态
					...
					switch (playmode){
						case GLOBAL_T.PLAYMODE.XueLiu://血流牌局过程中不能离开，除非破产了
							func_leave();
							break;

						case GLOBAL_T.PLAYMODE.XueZhan://血战胡过之后可以离开
							if(this.menu_ishu){
								// GLOBAL_OBJ.table.network.C2S.requestTableLeave(
								// 	MODEL_TABLEINFO.getRoomId(),
								// 	MODEL_TABLEINFO.getTableId(),
								...
								// );
								var miaoshu = "是否离开牌桌？";
								that.params.onClickLeave(miaoshu,3);
							}else{
								func_leave();
							}
							break;

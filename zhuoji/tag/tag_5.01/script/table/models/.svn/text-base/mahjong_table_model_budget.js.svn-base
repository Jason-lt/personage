    ...
    return PROTOTYPE.functionsExt(CACHE, {
        ...
        */
        ...
        getKongedSeatId:function(_localSeatId,tile){
            ...
            for(var i in kongList) {
                if(kongList[i].tile == tile) {
                    //等于自己就是补杠,当前状态没有区分明杠和补杠,这里自行判断一下;
        ...
        getKongedFromPos:function(_localSeatId,tile){
            ...
            //_seatId = GLOBAL_OBJ.table.utils.Seat.toLocalSeatId( _seatId );
            var str = "";
            switch(_seatId){
        ...
        getPlayerIndex:function(){
            ...
            for(var pl in CACHE){
                for (var i = 0; i < playerTotalScore.length; i++){
                    if (CACHE[pl].index == -1 && CACHE[pl].winScore == playerTotalScore[i]){
        ...
        /*
         庄闲、番型、额外番数、转弯豆、明豆、闷豆、冲锋/责任鸡、普通鸡、翻牌鸡 (这里主要是番型 额外番型)*/
        ...
        /*
         庄闲、番型、额外番数、1:转弯豆、2:明豆、3:闷豆、4:冲锋 5:责任鸡、6:普通鸡、7:翻牌鸡、 8:金鸡、 (这里主要是豆 鸡)*/
        getAllDouAndJi:function(_localSeatId){
            ...
            if (this.getMingDou(_localSeatId) != 0){
                douAndJi.push([2,this.getMingDou(_localSeatId)]);
            }
            if (this.getMenDou(_localSeatId) != 0){
                douAndJi.push([3,this.getMenDou(_localSeatId)]);
            }
            if (this.getChongFengJi(_localSeatId) != 0 ){
                douAndJi.push([4,this.getChongFengJi(_localSeatId)]);
            }
            if (this.getZeRenJi(_localSeatId) != 0){
                douAndJi.push([5,this.getZeRenJi(_localSeatId)]);
            }
            if (this.getPuTongJi(_localSeatId) != 0){
                if (this.isHaveJinJi(_localSeatId) != true) {
                    douAndJi.push([6,this.getPuTongJi(_localSeatId)]);
                }
            }
            if (this.getFanPaiJi(_localSeatId) != 0){
                if (this.isHaveJinJi(_localSeatId) != true) {
                    douAndJi.push([7,this.getFanPaiJi(_localSeatId)]);
                }
            }
            if (this.getJinJi(_localSeatId) != 0){
                douAndJi.push([8,this.getJinJi(_localSeatId)]);
            }
            return douAndJi
        },
        ...
        getGameFlow:function(_localSeatId){
            return CACHE[_localSeatId].bFlow;
        },
        ...
        dealWithWinModeInPatterns:function ( _localSeatId, _dahuPattern ) {
            ...
            /*fanPattern： [name,番数]  胡牌番型，最大胡牌番型  [‘平胡’,1]，['大对子’,5]，[‘清一色’,10]，
                [‘单吊’,10]， [‘双清’,20]（清一色单吊）[‘清大对’,15]，[‘七对’,10]，['龙七对’,20]，['清七对’,20]，[‘青龙背’,30]
            extPattern:  [[x,x]] [‘自摸’,1]，['杠上胡’,3]，[‘热炮’,1]，
                ['抢杠胡’,9]，[‘硬报’,20]，['杀硬报’,20]，[‘软报’,10], ['杀软报’,10]*/
        ...
        getPatternCCBByWinMode:function (_winMode) {
            ...
            if (_winMode == 0) { // 自摸
                ...
                tx_ccb = GLOBAL_OBJ.RES.XZ_ZM_TX_PXTX_TH_CCBI;
            }
            else if (_winMode == 12) { // 地胡
                tx_ccb = GLOBAL_OBJ.RES.XZ_ZM_TX_PXTX_DH_CCBI;
            }

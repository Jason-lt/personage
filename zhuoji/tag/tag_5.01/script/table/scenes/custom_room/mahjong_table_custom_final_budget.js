 ...
 *  mahjong_create_room_final_result_window.js
    mahjong_create_room_final_result_window
 *  mahjong
 	结算 window
 *
...
(function(){
	...
	GLOBAL_OBJ.table.scenes.Custom.Final = GLOBAL_OBJ.bkernel.base.BaseWindowController.extend({
		...
		onLoad: function() {
			...
			// GLOBAL_OBJ.LOGD(this._TAG,"_contentWidth : " + _contentWidth);
			var _percentage = MODEL_TABLEINFO.getSeatCount()/4;
			// GLOBAL_OBJ.LOGD(this._TAG,"_percentage : " + _percentage);
			var _oldPosX = this.nodeCenter.getPositionX();
			// GLOBAL_OBJ.LOGD(this._TAG,"_oldPosX : " + _oldPosX);
			var _newPosX=_oldPosX + _contentWidth * (1-_percentage) * 0.5;
			// GLOBAL_OBJ.LOGD(this._TAG,"_newPosX : " + _newPosX);
            ...
            GLOBAL_FUNCS.textureChange(this.imgTitle, GLOBAL_OBJ.RES.MAHJ_ROOM_TITLE_1_PNG);
	...
	// /*
	// @测试用例*/
	// GLOBAL_OBJ.table.scenes.Custom.Final.test = function(){
     //    GLOBAL_OBJ.bkernel.windows.Factory.produce(GLOBAL_OBJ.table.windows.consts.C_TABLE_BUDGET_FINAL_POYANG, {},

...
(function(){
	...
	GLOBAL_OBJ.table.scenes.Table.Mahjong.Mahj = cc.Class.extend({
		...
		mahjNotification:function () {
			GLOBAL_OBJ.bkernel.utils.Notification.listen( MAHJ_SAME_TILE_EVENTS.SHOW_SAME_TILE, this.showSameTile, this);
			GLOBAL_OBJ.bkernel.utils.Notification.listen( MAHJ_SAME_TILE_EVENTS.HIDE_SAME_TILE, this.hideSameTile, this);
		},
			...
			if (_params.tile == this.getTile()) {
				if(!this.isDestory){
					try {
						...
						GLOBAL_OBJ.LOGD(this._TAG, "花色：" + _params.tile + ";高亮显示。。。。。");
						this.doSameTile();
					}
					catch (e){
						GLOBAL_OBJ.LOGD(this._TAG, "showSameTile，执行异常");
						GLOBAL_OBJ.bkernel.utils.Notification.ignore( MAHJ_SAME_TILE_EVENTS.SHOW_SAME_TILE, this.showSameTile,this);
					}
			...
			if(!this.isDestory){
				...
				try {
					...
					if (_params.tile == 0){
						...
						if (_params.tile == this.getTile()) {
							GLOBAL_OBJ.LOGD(this._TAG, "花色：" + _params.tile + ";取消高亮。。。。。");
							this.cancelSameTile();
						}
					}
				}
				...
				GLOBAL_OBJ.bkernel.utils.Notification.ignore( MAHJ_SAME_TILE_EVENTS.HIDE_SAME_TILE, this.hideSameTile,this);
			}
		},

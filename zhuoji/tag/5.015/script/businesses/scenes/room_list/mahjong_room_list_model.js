...
(function () {
    ...
    GLOBAL_OBJ.businesses.scenes.RoomList.Model = (function(){
        ...
        return {
             ...
             * 解析房间实时人数消息
             */
            parseRoomOnline:function (_data) {
                GLOBAL_OBJ.LOGD(this._TAG, "解析房间实时人数消息");
                ...
                for (i = 0; i < rooms.length; i++){
                    roomObj = rooms[i];
                    if (roomObj.hasOwnProperty('mixedRoomId')){
                        mixId = roomObj['mixedRoomId'];
                    }
                    CACHE.roomsOnline[roomObj.roomId+""+mixId] = roomObj.onlineCount;
                }

                CACHE.matchsOnline = {};
                var matchs = _data.match;
                for (i = 0; i < matchs.length; i++){
                    roomObj = matchs[i];
                    CACHE.matchsOnline[roomObj.roomId + "_" + roomObj.matchId] = roomObj.onlineCount;
                }

                GLOBAL_OBJ.bkernel.utils.Notification.trigger( GLOBAL_OBJ.businesses.Events.UPDATE_ROOM_ONLINE, {});
            },

            getRoomOnlines:function (roomId, matchId) {
                var key = roomId + "";
                if (matchId){
                    key += "_" + matchId;
                }
                var onlineNum = -1;
                if (CACHE.roomsOnline.hasOwnProperty(key)){
                    onlineNum = CACHE.roomsOnline[key];
                }
                else if (CACHE.matchsOnline.hasOwnProperty(key)){
                    onlineNum = CACHE.matchsOnline[key];
                }
                return onlineNum;
            },

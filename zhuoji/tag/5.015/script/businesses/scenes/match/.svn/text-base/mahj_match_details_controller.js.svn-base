...
(function(){
	...
	GLOBAL_OBJ.businesses.scenes.Match.MatchDetailsController = GLOBAL_OBJ.bkernel.base.BaseController.extend({
		...
		_resetDate:function () {
			...
			if (this.isTimeMatch()){
				GLOBAL_OBJ.LOGD(this._TAG," _resetDate 请求定时赛房间数据");
				//请求定时赛房间数据
				GLOBAL_OBJ.businesses.network.C2S.requestTimeMatchDes(this._roomInfo.roomId);
				this['label_word_time_or_condition'].setString("开赛时间:");
			}
			else{
				GLOBAL_OBJ.LOGD(this._TAG," _resetDate 请求比赛房间数据");
				//请求比赛房间数据
				GLOBAL_OBJ.businesses.network.C2S.requestMatchDes(this._roomInfo.roomId, this._roomInfo.matchId);
				this['label_word_time_or_condition'].setString("开赛条件:");
			}
		},
		...
		onSignOut:function () {
			//定时赛退赛
			var that = this;
			GLOBAL_OBJ.LOGD(this._TAG," onSignOut 定时赛退赛");
			// this._showAlert("确定要退出比赛吗？", function () {
				...
				// GLOBAL_OBJ.LOGD(this._TAG," onSignOut 确定要退出比赛吗");
		...
		_resetInfoPanel:function () {
			...
			this._countDownPanel.rootNode.unscheduleAllCallbacks();
			this.matchDes = null;
		},
		...
		onSignup:function () {
			...
			if (this.isTimeMatch()){
				...
				GLOBAL_OBJ.LOGD(this._TAG, "onSignup  :  requestTimeMatchSignin");
			}
			else{
				GLOBAL_OBJ.LOGD(this._TAG, "onSignup  :  requestMatchSignin");
		...
		onClosed:function () {
			...
			// 		GLOBAL_OBJ.LOGD(this._TAG, "onClosed  :  关闭此窗口，会导致你无法进入比赛，确定关闭吗");
		...
		_initRecord:function () {
			...
			GLOBAL_OBJ.LOGD(this._TAG, "_initRecord = " + JSON.stringify(rankList));

    ...
    GLOBAL_OBJ.businesses.GameWorld = {
        ...
        */
        ...
        /*
         解析比赛场相关消息
         */
        parseRoom:function(_result){
            GLOBAL_OBJ.businesses.scenes.Match.Model.parseRoomFromMsg(_result);
        },

        /*
         解析比赛场相关消息(定时赛)
         */
        parseMatch:function(_result){
            GLOBAL_OBJ.businesses.scenes.Match.Model.parseRoomFromMatchMsg(_result)
        },

        /*

         */
        parseGame:function(_result){
            if (_result.action == 'room_online'){
                GLOBAL_OBJ.businesses.scenes.RoomList.Model.parseRoomOnline(_result);
            }
            else if (_result.action == 'get_exit_plugin_remind'){
                //退出插件挽留信息
                GLOBAL_OBJ.businesses.modules.Game.PluginRemindModel.parse(_result);
            }
            else{
                GLOBAL_OBJ.businesses.scenes.Match.Model.parseRoomFromMatchMsg(_result);
            }

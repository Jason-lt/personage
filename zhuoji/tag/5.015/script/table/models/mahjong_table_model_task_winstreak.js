    ...
    */
    ...
    return PROTOTYPE.functionsExt(CACHE, {
        ...
        */
        ...
        //获取当前奖励描述
        getCurStreakPrize:function(count){
            var prize = "";
            GLOBAL_OBJ.LOGD(this._TAG, "getCurStreakPrize ...count = " + count);
            for(var i = 0; i < CACHE.desc.length; ++i){
                GLOBAL_OBJ.LOGD(this._TAG, "getCurStreakPrize ...CACHE.desc[i].winCount = " + CACHE.desc[i].winCount);
                if (CACHE.desc[i].winCount && CACHE.desc[i].winCount == count){
                    prize = CACHE.desc[i].prize;
                    break;
                }
            }
            return prize
        ...
        //获取当前连胜次数奖励详情
        getCurStreakawardItems:function(count){
            var rewards = [];
            for(var i = 0; i < CACHE.desc.length; ++i){
                if (CACHE.desc[i].winCount && CACHE.desc[i].winCount == count){
                    var award = CACHE.desc[i].rewards;
                    rewards =  award.items;
                    break;
                }
            }
        ...
        //获取当前连胜是否存在奖励
        getIsHaveAward:function(){
            var isHaveAward = false;
            for(var i = 0; i < CACHE.desc.length; ++i){
                if (CACHE.desc[i].winCount && CACHE.desc[i].winCount == CACHE.winCount){
                    isHaveAward = true;
                    break;
                }
            }
        ...
        loopAcviveTransition:function () {
            var _data = {};
            _data['current'] = CACHE.winCount;
            _data['nextNum'] = CACHE.winCount;
            if (CACHE.winCount > 0) {
                _data['current'] = CACHE.winCount - 1;
            }else {
                _data['current'] = 0;
            }
            return _data;

...
(function(){
	...
	GLOBAL_OBJ.table.windows.Portrait.Common = GLOBAL_OBJ.bkernel.base.BaseWindowController.extend({
		...
		onLoad: function() {
			...
			GLOBAL_OBJ.bkernel.utils.Notification.listen(GLOBAL_OBJ.businesses.Events.UPDATE_MAHJ_MATCH_REAL_RANK, this.showRealRank, this);
			...
			// 定缺提示
			GLOBAL_OBJ.bkernel.utils.Notification.listen(GLOBAL_OBJ.table.Events.UPDATE_SHOW_ABSENCE_FIXD, this.showAbsence, this);
		...
		showRealRank:function () {
			GLOBAL_OBJ.LOGD(this._TAG, "比赛桌实时排名 showRealRank this.params.uid = " + this.params.uid);
			var rank =  MODEL_MATCH.getRealRank(this.params.uid);
			GLOBAL_OBJ.LOGD(this._TAG, "比赛桌实时排名 showRealRank rank = " + rank);
			if (this['matchRank']) {

			}
			this['matchRank'].setVisible(rank > 0);
			// this['matchRank'].setVisible(true);
			if (this['matchRank'].isVisible()){
				GLOBAL_FUNCS.textureChange(this['matchRank'], GLOBAL_OBJ.RES['MAHJ_MATCH_TABLE_RANK_' + rank + '_PNG']);
			}
		},
		...
		*/
		...
		updateScore:function () {
			...
			else if (MODEL_TABLEINFO.getTableType() == GLOBAL_OBJ.TableType.Match){
				score = MODEL_USERS.getCustomScore(this.params.uid);
				scoreText = score + "";
			}
			else{
        ...
        showAbsence:function (_data) {
			if (this.params.sid == _data.seatId) {
				if (_data.dqType == -1) {
					return;
				}

				GLOBAL_OBJ.LOGD(this._TAG, "_head_show_absence_");
				...
				GLOBAL_FUNCS.textureChange(this.dingqueSpr, wtt[_data.dqType]);
				this.dingqueSpr.setVisible(true);
			}
        },

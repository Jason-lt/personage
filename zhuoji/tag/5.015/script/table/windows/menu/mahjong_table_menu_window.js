			...
			if (this.isCreate){
				...
				if(this.menuState == "waitting"){//等待状态
					...
					var callback = function () {
						...
						var winCount = MODEL_TASK_WINSTREAK.getWinStreakCount();
						var winTimes = MODEL_TASK_LOOPWINTIMESTASK.getLoopWinTimesPlayTimes();
						if (loopTimes > 0) {
							...
							if(chaTime == 0){
								miaoshu = "再玩" + loopTimes + "局即可获得" + award + "奖励";
							}else{
								miaoshu = "您已经玩了" + playTimes + "局，再玩" + chaTime + "局即可获得" + award + "奖励";
							}
							that.params.onClickLeave(miaoshu, 1 );
						}else if (winCount > 0 && MODEL_TASK_WINSTREAK.getWinStreaks()) {
							miaoshu = "您已经完成了" + winCount + "连胜";
							var prize = MODEL_TASK_WINSTREAK.getCurStreakPrize(winCount + 1);
							if (prize != "") {
								miaoshu = miaoshu + "\n" + (winCount+1) + "连胜奖励:" + prize;
							}
							that.params.onClickLeave(miaoshu, 1 );
						}else if (winTimes > 0 && MODEL_TASK_LOOPWINTIMESTASK.getIsHaveLoopWinTimes()) {
							var roundCont = MODEL_TASK_LOOPWINTIMESTASK.getLoopWinTimesRoundCount();
							var chaTime = roundCont - winTimes;
							var award = MODEL_TASK_LOOPWINTIMESTASK.getLoopAcviveAward();
							if(chaTime == 0){
								miaoshu = "再赢" + roundCont + "局即可获得" + award + "奖励";
							}else{
								miaoshu = "您已经赢了" + winTimes + "局," + "再赢" + chaTime + "局即可获得" + award + "奖励";
							}
							that.params.onClickLeave(miaoshu, 1 );
						}else {
							var miaoshu = "是否离开牌局";

        ...
        for root, dirs, files in os.walk(imgPath):
            for file in files:
                if isImg(file):
                    ...
                    ccbPaths = ccbPath.split(",")
                    useage = 0
                    for pathFile in ccbPaths:
                        array = searchPngFromDirs(pathFile,file)
                        useage +=len(array)
                        for i in array:
                            writeFile.write("    " + i+"\n")

                    if useage == 0 :
                        writeFile.write("notUse " + root+"/"+file+ "\n" )
                    else :
                        writeFile.write("use " + str(useage) +" "+root+"/"+ file+ "\n" )
                    # for ccb in array:
                    #     split = ccb.split("_")
                    #     # print "split"+ split[0]
                    #     # print "split"+ split[1] 
                    ...
                    #     writeFile.write(format.format(file,ext,relPath,ccb,split[0],split[1],"main",modify))
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print '需要带ccb根目录的路径,二,三 和 大厅图片散图路径 和 导出文件'
    elif len(sys.argv) == 3:
